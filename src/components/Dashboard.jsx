...
class Dashboard extends Component {
	constructor(props, context) {
		...
		this.events = this.contracts["OpenEvents"].methods.eventsOf.cacheCall(
	...
	render() {
		...
		if (
			typeof this.props.contracts["OpenEvents"].eventsOf[this.events] !==
			"undefined"
		) {
			let eventCount = this.props.contracts["OpenEvents"].eventsOf[
				this.events
			].value;
			let eventCache = [];
			let eventDetails = [];
			...
			for (var i = 0; i < eventCount.length; i++) {
				eventCache.push(
					this.contracts["OpenEvents"].methods.getEvent.cacheCall(
						eventCount[i]
					)
				);
				if (
					typeof this.props.contracts["OpenEvents"].getEvent[
						eventCache[i]
					] !== "undefined" &&
					this.props.contracts["OpenEvents"].getEvent[eventCache[i]]
						.value
				) {
					eventDetails.push({
						result: this.props.contracts["OpenEvents"].getEvent[
							eventCache[i]
						].value,
						id: eventCount[i],
					});
						...
						<div className="row user-list mt-4">
							...
							<div className="col-lg-4 pb-4 d-flex align-items-stretch">
								<div className="dashboard-card">
									...
									>
										<a
											href="https://app.uniswap.org/#/swap?inputCurrency=ETH&outputCurrency=0xfe1b6abc39e46cec54d275efb4b29b33be176c2a"
											target="blank"
											className="mt-10"
										>
											<p className="dashboard-uniswap">
												<i className="fas fa-sync"></i>{" "}
												BUY PHNX WITH UNISWAP
											</p>
										</a>
									</div>
								</div>
